...
MODEL_MAP = {**TOGETHER_MODEL_MAP, **NVIDIA_MODEL_MAP}
...
def get_openai_model_args(model_args, model=None):
    ...
    if model_args is not None:
        ...
        if "frequency_penalty" in model_args and (
            not model or model not in list(NVIDIA_MODEL_MAP.values())
        ):
            openai_model_args["frequency_penalty"] = model_args["frequency_penalty"]
        if "presence_penalty" in model_args and (
            not model or model not in list(NVIDIA_MODEL_MAP.values())
        ):
            openai_model_args["presence_penalty"] = model_args["presence_penalty"]
        if (
            "stop" in model_args
            and model_args["stop"] is not None
            and (not model or model not in list(NVIDIA_MODEL_MAP.values()))
        ):
...
async def openai_chat_completion(
    client, messages, model="gpt-3.5-turbo", model_args=None
):
    openai_model_args = get_openai_model_args(model_args, model)
    text = ""
    exception = None
    response = await client.chat.completions.create(
        model=model, messages=messages, **openai_model_args
    )
...
async def multi_turn_inference_openai_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    ...
    for prompt in user_prompt:
        prompt = eval(prompt) if is_evaluable(prompt) else prompt
        messages.append({"role": "user", "content": prompt.strip()})
        response = await openai_chat_completion(
            client, messages, model=model, model_args=model_args
        )
...
async def inference_openai_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    if isinstance(user_prompt, list):
        return await multi_turn_inference_openai_model(
            client, model, user_prompt, system_prompt, model_args
        )
...
async def reka_chat_completion(client, messages, model="reka-core", model_args=None):
    ...
    response = await client.chat.create(
        model=model, messages=messages, **reka_model_args
    )
...
async def inference_reka_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    content = ""
    messages = []
    ...
    return await reka_chat_completion(
        client, messages, model=model, model_args=model_args
    )
...
async def zhipu_chat_completion(client, messages, model="glm-4", model_args=None):
    ...
    response = client.chat.completions.create(
        model=model, messages=messages, **zhipu_model_args
    )
    content = response.choices[0].message.content
...
async def inference_zhipu_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    messages = []
    ...
    return await zhipu_chat_completion(
        client, messages, model=model, model_args=model_args
    )
...
async def ai21_chat_completion(
    client, messages, model="jamba-1.5-mini", model_args=None
):
    ai21_model_args = get_ai21_model_args(model_args)
    text = ""
    exception = None
    response = client.chat.completions.create(
        model=model, messages=messages, **ai21_model_args
    )
    content = response.choices[0].message.content
...
async def inference_ai21_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    messages = []
    ...
    return await ai21_chat_completion(
        client, messages, model=model, model_args=model_args
    )
...
async def azure_chat_completion(
    client, messages, model="Phi-3-small-instruct-8k", model_args=None
):
...
async def inference_azure_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    messages = []
    ...
    return await azure_chat_completion(
        client, messages, model=model, model_args=model_args
    )
...
async def cohere_chat_completion(
    client, messages, model="Phi-3-small-instruct-8k", model_args=None
):
...
async def inference_cohere_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    messages = []
    ...
    return await cohere_chat_completion(
        client, messages, model=model, model_args=model_args
    )
...
async def mistral_chat_completion(
    client, messages, model="mistral-small-latest", model_args=None
):
    mistral_model_args = get_mistral_model_args(model_args)
    text = ""
    exception = None
    response = client.chat.complete(
        model=model, messages=messages, **mistral_model_args
    )
    content = response.choices[0].message.content
...
async def inference_mistral_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    messages = []
...
async def anthropic_chat_completion(
    ...
    try:
        if system_prompt:
            ...
            response = await client.messages.create(
                model=model, messages=messages, **anthropic_model_args
            )
...
async def inference_anthropic_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
...
async def inference_google_model(
    client, model, user_prompt, system_prompt=None, model_args=None
):
    text = ""
    exception = None
...
def load_model(model_path="gpt2", model_args=None, cache_dir=None, device="cuda"):
    tokenizer_path = model_path
    tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_path, cache_dir=cache_dir, trust_remote_code=True
    )
...
def get_hf_model_args(model_args):
    ...
    if model_args is not None:
        ...
        if "typicality" in model_args:
            hf_model_args["typical_p"] = model_args["typicality"]
        if "min_p" in model_args:
            hf_model_args["min_p"] = model_args["min_p"]
        if (
            hf_model_args["temperature"] == 0
            and hf_model_args["top_p"] == 1
            and hf_model_args["typical_p"] == 1
            and not hf_model_args.get("top_k")
            and not hf_model_args.get("min_p")
        ):
...
def inference_hf_model(
    ...
    if model_name.startswith("baichuan"):
        input_ids = [
            build_baichuan_chat_input(
                model, tokenizer, messages, hf_model_args["max_new_tokens"]
            ).to(device)
...
async def inference_api_model(client, model, batch, model_args=None):
    tasks = []
...
def configure_openai_client(model, api_key, is_openai_azure=False):
    if is_openai_azure:
        endpoint = os.getenv(
            "AZURE_OPENAI_API_ENDPOINT", "https://sigturk-openai.openai.azure.com/"
        )
        ...
        if model in HF_INFERENCE_MODELS:
            client = AsyncOpenAI(
                ...
                api_key=api_key if api_key is not None else os.getenv("HF_API_KEY"),
            )
        elif model in TOGETHER_MODELS:
            client = AsyncOpenAI(
                base_url="https://api.together.xyz/v1",
                api_key=(
                    api_key if api_key is not None else os.getenv("TOGETHER_API_KEY")
                ),
            )
            ...
            client = AsyncOpenAI(
                ...
                api_key=api_key if api_key is not None else os.getenv("LAMBDA_API_KEY"),
            )
        else:
            client = AsyncOpenAI(
...
def configure_reka_client(api_key):
    return AsyncReka(
        api_key=api_key if api_key is not None else os.getenv("REKA_API_KEY")
    )


def configure_zhipu_client(api_key):
    return ZhipuAI(
        api_key=api_key if api_key is not None else os.getenv("ZHIPU_API_KEY")
    )


def configure_ai21_client(api_key):
    return AI21Client(
        api_key=api_key if api_key is not None else os.getenv("AI21_API_KEY")
    )
...
def configure_azure_client(api_key):
    return ChatCompletionsClient(
        ...
        credential=AzureKeyCredential(
            api_key if api_key is not None else os.getenv("AZURE_INFERENCE_API_KEY")
        ),
    )
...
def configure_cohere_client(api_key):
    return CohereClient(
        api_key=api_key if api_key is not None else os.getenv("COHERE_API_KEY")
    )


def configure_mistral_client(api_key):
    return Mistral(
        api_key=api_key if api_key is not None else os.getenv("MISTRAL_API_KEY")
    )
...
async def inference_on_datapath(
    args,
    datapath,
    client=None,
    model=None,
    ...
    outputs = {
        "metadata": {
            **input_data["metadata"],
            "source": datapath,
            "size": len(data),
            "unique_size": len(data) // args.num_inferences,
            "per_sample_num_inferences": args.num_inferences,
            "model": args.model,
    ...
    if args.resume:
        ...
        output_path = os.path.join(
            args.output_dir, f"{datapath.stem}_{model_name}_{unique_id}.json"
        )
        error_path = os.path.join(
            args.output_dir, f"{datapath.stem}_{model_name}_{unique_id}_errors.txt"
        )
    ...
    for batch in tqdm(
        batched(data, size=args.batch_size),
        ...
        try:
            ...
            if model_name in API_MODELS:
                results = await inference_api_model(
                    client, model_name, filtered_batch, model_args
                )
            else:
                results = inference_hf_model(
                    model,
                    tokenizer,
                    filtered_batch,
                    model_name=model_name,
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Path to evaluation data file(s) in json or directory",
        required=True,
    )
    parser.add_argument("-a", "--api-key", type=str, help="Model API Key")
    parser.add_argument(
        "-oa", "--openai-azure", action="store_true", help="If OpenAI on Azure"
    )
    parser.add_argument(
        "-m", "--model", type=str, help="Model to use for evaluation", default="gpt-4"
    )
    parser.add_argument(
        "-t",
        "--temperature",
    ...
    parser.add_argument(
        "-p", "--top-p", type=float, help="Top-p for generation", default=1
    )
    parser.add_argument(
        "-mp", "--min-p", type=none_or_float, help="Min-p for generation", default=None
    )
    parser.add_argument(
        "-k", "--top-k", type=none_or_int, help="Top-k for generation", default=None
    )
    parser.add_argument(
        "-y", "--typicality", type=float, help="Typicality for generation", default=1.0
    )
    parser.add_argument(
        "-fp",
        "--frequency-penalty",
    ...
    parser.add_argument(
        "-n", "--num-samples", type=int, help="Number of samples to evaluate", default=0
    )
    parser.add_argument(
        "-c", "--cache-dir", type=str, help="Cache directory for model", default=None
    )
    parser.add_argument(
        "-b", "--batch-size", type=int, help="Batch size for evaluation", default=1
    )
    parser.add_argument(
        "-r",
        "--resume",
        action="store_true",
        help="Resume evaluation from the current file",
    )
    parser.add_argument(
        "-s", "--stop", type=str, help="Stop token for generation", default=None
    )
    parser.add_argument(
        "-nf",
        "--num-inferences",
        type=int,
        help="Number of inferences per sample",
        default=1,
    )
    parser.add_argument(
        "-et",
        "--enable_thinking",
        action="store_true",
        help="Toggle thinking mode during generation for models that support it",
    )
    ...
    if args.model in API_MODELS:
        if args.model in OPENAI_COMPATIBLE_MODELS:
            client = configure_openai_client(
                args.model, args.api_key, args.openai_azure
            )
        ...
        model, tokenizer = load_model(
            args.model, cache_dir=args.cache_dir, device=args.device
        )
        # Nonchat models still need a chat template set
        # So this template has no role tags
    ...
    for datapath in args.datapaths:
        if pathlib.Path(datapath).is_file():
            ...
            datapaths.extend(find_files(datapath, "json"))

    for datapath in datapaths:
        await inference_on_datapath(
            args,
            datapath,
            client=client,
            model=model,
